...
--joins
select C.first_name, C.last_name, C.email, A.account_no, A.balance from clients as C left join accounts as A on A.client_id = C.id;

select A.account_no as cuenta, A.client_id as id_cliente, AcT."name" as tipo_cuenta from accounts as A join account_types as AcT on A."type" = AcT.id;

select T.id, T.amount, TT."name"  from transactions as T join transactions_types as TT on T.transaction_type = TT.id;

select T.id, T.amount, TT."name"  from transactions as T right join transactions_types as TT on T.transaction_type = TT.id;


--views
create view v_clients_accounts as select C.first_name, C.last_name, C.email, A.account_no, A.balance from clients as C left join accounts as A on A.client_id = C.id;

create view v_accounts_types as select A.account_no as cuenta, A.client_id as id_cliente, AcT."name" as tipo_cuenta from accounts as A join account_types as AcT on A."type" = AcT.id;

create view v_transactions_detail as select T.id, T.amount, TT."name"  from transactions as T join transactions_types as TT on T.transaction_type = TT.id;

create view v_transations_types_detail as select T.id, T.amount, TT."name"  from transactions as T right join transactions_types as TT on T.transaction_type = TT.id;